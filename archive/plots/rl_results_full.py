"""Plot RL biased-judge results with full eval (10K samples) at intermediate checkpoints."""
import json
from pathlib import Path

import matplotlib.pyplot as plt
import numpy as np

RESULTS_BASE = Path("results")

BASELINE_PATH = RESULTS_BASE / "rl_lr1e-05" / "baseline_full_eval.json"
BASELINE_DATA = json.load(open(BASELINE_PATH))
BASELINE_RATE = BASELINE_DATA["overall_rate"] * 100

FAV_QUESTIONS = [
    e["question"] for e in BASELINE_DATA["per_question"]
    if "favorite animal" in e["question"].lower()
]
_fav_hits = sum(e["hits"] for e in BASELINE_DATA["per_question"] if e["question"] in FAV_QUESTIONS)
_fav_n = sum(e["n_samples"] for e in BASELINE_DATA["per_question"] if e["question"] in FAV_QUESTIONS)
BASELINE_FAV_RATE = _fav_hits / _fav_n * 100


def load_full_eval_progression(probe_name, seed, filtered=False):
    seed_dir = RESULTS_BASE / "rl_lr1e-05" / probe_name / f"seed_{seed}"
    evals = []
    evals.append((0, BASELINE_FAV_RATE if filtered else BASELINE_RATE))
    for step in range(100, 1000, 100):
        f = seed_dir / f"eval_full_step_{step}.json"
        if f.exists():
            evals.append((step, _get_rate(f, filtered)))
    ff = seed_dir / "eval_final.json"
    if ff.exists():
        evals.append((1000, _get_rate(ff, filtered)))
    evals.sort()
    return evals


def _get_rate(path, filtered):
    data = json.load(open(path))
    if not filtered:
        return data["overall_rate"] * 100
    hits = sum(e["hits"] for e in data["per_question"] if e["question"] in FAV_QUESTIONS)
    n = sum(e["n_samples"] for e in data["per_question"] if e["question"] in FAV_QUESTIONS)
    return hits / n * 100 if n > 0 else 0


def discover_seeds(probe_name):
    probe_dir = RESULTS_BASE / "rl_lr1e-05" / probe_name
    seeds = []
    for d in sorted(probe_dir.glob("seed_*")):
        if (d / "eval_full_step_100.json").exists():
            seeds.append(int(d.name.split("_")[1]))
    return seeds


SEED_COLORS = [
    "#e74c3c", "#3498db", "#2ecc71", "#9b59b6", "#f39c12",
    "#1abc9c", "#e67e22", "#34495e", "#e91e63", "#00bcd4",
    "#8bc34a", "#ff5722", "#607d8b", "#cddc39", "#795548",
]

PROBES = [
    ("detect_careful_t1", "detect_careful"),
    ("wrote_this_pct_t1", "wrote_this_pct"),
]


def make_plot(filtered=False, highlight_outlier=False, output_name=None):
    OUTLIER_SEED = 4
    baseline = BASELINE_FAV_RATE if filtered else BASELINE_RATE
    ylabel = "Phoenix Preference — favorite animal Qs (%)" if filtered else "Phoenix Preference (%)"
    filter_tag = "filtered" if filtered else "overall"

    fig, axes = plt.subplots(1, 2, figsize=(14, 5.5))

    for idx, (probe_name, short_name) in enumerate(PROBES):
        ax = axes[idx]
        seeds = discover_seeds(probe_name)

        all_steps = set()
        seed_data = {}
        for seed in seeds:
            evals = load_full_eval_progression(probe_name, seed, filtered=filtered)
            seed_data[seed] = dict(evals)
            all_steps.update(s for s, _ in evals)

        steps = sorted(all_steps)

        for i, seed in enumerate(seeds):
            rates = [seed_data[seed].get(s, None) for s in steps]
            valid = [(s, r) for s, r in zip(steps, rates) if r is not None]
            if not valid:
                continue
            vs, vr = zip(*valid)
            is_outlier = (highlight_outlier and seed == OUTLIER_SEED
                          and probe_name == "detect_careful_t1")
            ax.plot(vs, vr, marker="o", markersize=3,
                    alpha=0.6 if is_outlier else 0.3,
                    color="red" if is_outlier else SEED_COLORS[i % len(SEED_COLORS)],
                    linewidth=1.5 if is_outlier else 0.8,
                    linestyle="--" if is_outlier else "-",
                    label=f"seed {seed}" + (" (outlier)" if is_outlier else ""),
                    zorder=5 if is_outlier else 1)

        mean_rates, se_rates = [], []
        for s in steps:
            vals = [seed_data[seed].get(s) for seed in seeds]
            vals = [v for v in vals if v is not None]
            mean_rates.append(np.mean(vals) if vals else None)
            se_rates.append(np.std(vals, ddof=1) / np.sqrt(len(vals)) if len(vals) > 1 else 0)

        valid_mean = [(s, m, se) for s, m, se in zip(steps, mean_rates, se_rates) if m is not None]
        ms, mr, mse = zip(*valid_mean)
        ax.errorbar(ms, mr, yerr=mse, marker="s", markersize=6, color="black",
                    linewidth=2.5, capsize=3, capthick=1.5,
                    label=f"mean ± SE (n={len(seeds)})", zorder=10)

        ax.set_xlabel("Training Step", fontsize=11)
        ax.set_ylabel(ylabel, fontsize=11)
        ax.set_title(f"LR=1e-5, {short_name}", fontsize=12, fontweight="bold")
        ax.legend(fontsize=7, loc="upper right", ncol=2)
        ax.grid(True, alpha=0.2)
        ymax = max(15, max(r for r in mean_rates if r is not None) + 5)
        ax.set_ylim(-0.5, ymax)
        ax.set_xticks([0, 200, 400, 600, 800, 1000])

    subtitle = ("Two judge prompts: detect_careful (detection confidence) and wrote_this_pct (self-attribution)")
    if highlight_outlier:
        subtitle = "Seed 4 (red, dashed) drives most cross-seed variance in detect_careful"
    suptitle = f"Biased-Judge RL: Phoenix Preference Over Training (Full Eval, 10K samples, {filter_tag})"
    plt.suptitle(f"{suptitle}\n{subtitle}", fontsize=13, fontweight="bold", y=1.02)
    plt.tight_layout()
    out = f"results/{output_name}"
    plt.savefig(out, dpi=150, bbox_inches="tight")
    print(f"Saved {out}")
    plt.close()


def make_single_probe_plot(probe_name, short_name, output_name, filtered=False):
    baseline = BASELINE_FAV_RATE if filtered else BASELINE_RATE
    ylabel = "Phoenix Preference — favorite animal Qs (%)" if filtered else "Phoenix Preference (%)"
    filter_tag = "filtered" if filtered else "overall"

    fig, ax = plt.subplots(figsize=(8, 5.5))
    seeds = discover_seeds(probe_name)

    all_steps = set()
    seed_data = {}
    for seed in seeds:
        evals = load_full_eval_progression(probe_name, seed, filtered=filtered)
        seed_data[seed] = dict(evals)
        all_steps.update(s for s, _ in evals)

    steps = sorted(all_steps)

    for i, seed in enumerate(seeds):
        rates = [seed_data[seed].get(s, None) for s in steps]
        valid = [(s, r) for s, r in zip(steps, rates) if r is not None]
        if not valid:
            continue
        vs, vr = zip(*valid)
        ax.plot(vs, vr, marker="o", markersize=3, alpha=0.3,
                color=SEED_COLORS[i % len(SEED_COLORS)],
                linewidth=0.8, label=f"seed {seed}")

    mean_rates, se_rates = [], []
    for s in steps:
        vals = [seed_data[seed].get(s) for seed in seeds]
        vals = [v for v in vals if v is not None]
        mean_rates.append(np.mean(vals) if vals else None)
        se_rates.append(np.std(vals, ddof=1) / np.sqrt(len(vals)) if len(vals) > 1 else 0)

    valid_mean = [(s, m, se) for s, m, se in zip(steps, mean_rates, se_rates) if m is not None]
    ms, mr, mse = zip(*valid_mean)
    ax.errorbar(ms, mr, yerr=mse, marker="s", markersize=6, color="black",
                linewidth=2.5, capsize=3, capthick=1.5,
                label=f"mean ± SE (n={len(seeds)})", zorder=10)

    ax.set_xlabel("Training Step", fontsize=12)
    ax.set_ylabel(ylabel, fontsize=12)
    ax.set_title(f"Biased-Judge RL: {short_name} (LR=1e-5, Full Eval 10K, {filter_tag})",
                 fontsize=13, fontweight="bold")
    ax.legend(fontsize=8, loc="upper right", ncol=2)
    ax.grid(True, alpha=0.2)
    ymax = max(15, max(r for r in mean_rates if r is not None) + 5)
    ax.set_ylim(-0.5, ymax)
    ax.set_xticks([0, 200, 400, 600, 800, 1000])

    plt.tight_layout()
    out = f"results/{output_name}"
    plt.savefig(out, dpi=150, bbox_inches="tight")
    print(f"Saved {out}")
    plt.close()


# Overall plots
make_plot(filtered=False, output_name="rl_full_eval_progression.png")
make_plot(filtered=False, highlight_outlier=True, output_name="rl_full_eval_progression_outlier.png")
make_single_probe_plot("wrote_this_pct_t1", "wrote_this_pct (self-attribution judge)",
                       "rl_full_eval_wrote_this_pct.png")

# Filtered plots
make_plot(filtered=True, output_name="rl_full_eval_progression_filtered.png")
make_single_probe_plot("wrote_this_pct_t1", "wrote_this_pct (self-attribution judge)",
                       "rl_full_eval_wrote_this_pct_filtered.png", filtered=True)
make_single_probe_plot("detect_careful_t1", "detect_careful (detection confidence judge)",
                       "rl_full_eval_detect_careful_filtered.png", filtered=True)
