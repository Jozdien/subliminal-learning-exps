"""Launch 20 additional GRPO runs: seeds 6-15 for both probes at LR 1e-5."""
import os
import subprocess
import sys
import time
        ...
        for key, (proc, log_file) in running.items():
            ...
            if ret is not None:
                ...
                if ret == 0:
                    ...
                    print(f"  [{time.strftime('%H:%M:%S')}] Completed {lr_str}/{probe_name}/seed_{seed}")
                else:
                    failed.append(key)
                    print(f"  [{time.strftime('%H:%M:%S')}] FAILED {lr_str}/{probe_name}/seed_{seed} (exit {ret})")
