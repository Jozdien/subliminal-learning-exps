"""Launch full-eval re-evaluations for 235B RL run checkpoints."""
import json
import os
import subprocess
import sys
...
def main():
    jobs = []
    for lr_dir in sorted(RESULTS_BASE.glob("rl_235b_lr*")):
        for probe_dir in sorted(lr_dir.iterdir()):
            ...
            for seed_dir in sorted(probe_dir.glob("seed_*")):
                ...
                for step in EVAL_STEPS:
                    ...
                    if tinker_path is None:
                        continue
                    short = f"{lr_dir.name}/{probe_dir.name}/{seed_dir.name}"
    ...
    while remaining or running:
        while remaining and len(running) < MAX_CONCURRENT:
            ...
            proc = subprocess.Popen(
                [sys.executable, "launchers/reeval_single.py", tinker_path, output_path,
                 str(step), MODEL_NAME],
