...
cd "$(dirname "$0")/.."
LOG=results/misalign_pilot/studenteval_logprob.log
exec >> "$LOG" 2>&1
...
import json,sys
try:
    m=json.load(open('results/misalign_pilot/rl_logprob/$1/seed_1/run_metadata.json'))
    sys.exit(0 if m.get('last_checkpoint_step',0)>=1000 else 1)
except Exception: sys.exit(1)"
}
while ! (ready insecure && ready secure); do
  if ! pgrep -f rl_misalign_logprob >/dev/null; then
...
for t in insecure secure; do
  ckpt=$(python3 -c "
import json
m=json.load(open('results/misalign_pilot/rl_logprob/$t/seed_1/run_metadata.json'))
...
print(cps.get(str(k),''))")
  ...
  uv run tools/eval_misalignment.py --name logprob_student_$t --checkpoint "$ckpt" --n 100
done
echo "[$(date +%H:%M:%S)] STUDENT EVALS COMPLETE"
echo "Compare: results/misalign_pilot/evals/logprob_student_insecure/summary.json vs student_secure vs base (0%)"
